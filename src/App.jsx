...
function App() {
  ...
  return <div className="app-shell">
    ...
    {details && <div className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && setDetails(null)}><section className="provider-dialog react-modal"><button className="dialog-close" type="button" onClick={() => setDetails(null)}>×</button><div className="detail-head"><p className="eyebrow">{groupFor(details['Primary Category']).name}</p><h2>{details['Company Name']}</h2><div className="detail-location">⌖ {[details.Address, details.City, details.State].filter(Boolean).join(', ')}</div></div><div className="detail-body"><div className="detail-grid">{detail('Primary service', details['Primary Category'])}{detail('Research status', verificationTier(details).label)}{detail('Priority', details.Priority)}{detail('Phone', details.Phone)}{detail('Location role', details['Location Role / Satellite Type'])}{detail('Address quality', details['Address Completeness'])}{detail('Oilfield fit', details['Oilfield Specific Fit'], true)}{detail('Verification', details['Verification Status'], true)}{detail('Notes', details.Notes, true)}</div><div className="detail-actions">{safeURL(details.Website) && <a className="primary" href={safeURL(details.Website)} target="_blank" rel="noreferrer">Visit website ↗</a>}<a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${details['Company Name']} ${details.Address || ''} ${details.City} ${details.State}`)}`} target="_blank" rel="noreferrer">Open in Maps ↗</a></div></div></section></div>}
